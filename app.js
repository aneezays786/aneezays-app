...
let cart=JSON.parse(localStorage.getItem("aneezaysCart")||"[]");
...
function renderCart(){
 ...
 if(!cart.length){box.innerHTML='<p style="color:#766c66;font-size:13px;padding:25px 0">Your bag is waiting for something beautiful. ♡</p>'}
 else box.innerHTML=cart.map(x=>`<div class="cart-row"><div class="mini-img"></div><div><h4>${x.name}</h4><small>${money(x.price)} · Qty ${x.qty}</small><br><button class="remove" onclick="removeCart(${x.id})">Remove</button></div><b>${money(x.price*x.qty)}</b></div>`).join("");
...
document.getElementById("checkoutBtn").onclick=()=>alert(cart.length?"Checkout is ready for the next step. Connect your payment/order system here.":"Your bag is empty.");
